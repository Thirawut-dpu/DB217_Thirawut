let userScore = 0;
let compScore = 0;
const userScore_span = document.querySelector('#user-score'); // const = ค่าเดียวไม่เปลี่ยน querySelector = แค่ตัวเดียว
...
function win(userChoice, compChoice) {
    ...
    result_div.innerHTML = `${userChoice} beats ${compChoice}. You win!` //แสดงผลลัพธ์
    user_choice.classList.add(`winner`);
}

function loses(userChoice, compChoice) {
    userScore_span.innerHTML = ++compScore;
    result_div.innerHTML = `${compChoice} beats ${userChoice}. You lose!` //แสดงผลลัพธ์
    comp_choice.classList.add(`winner`);
}

function draw(chioce) {
    result_div.innerHTML = `It was a draw! You both chose ${chioce}` //แสดงผลลัพธ์
}
